...
RoundUp (unsigned int nominator, unsigned int denominator)
{
 ...
 */
void
SavePGMFile (const unsigned char *data, size_t width, size_t height,
	     const char *filename)
{
...
DEFINE_int32 (random_seed, -1,
	      "Override random seed (default uses std::random_device)");
DEFINE_int32 (classify, -1,
	      "Number of images to classify to compute error rate (default uses entire test set)");
...
DEFINE_string (train_images, "train-images-idx3-ubyte",
	       "Training images filename");
DEFINE_string (train_labels, "train-labels-idx1-ubyte",
	       "Training labels filename");
...
struct ConvBiasLayer
{
  ...
  int in_width, in_height, out_width, out_height;
    ...
    ConvBiasLayer (int in_channels_, int out_channels_, int kernel_size_,
		   int in_w_,
		   int in_h_):pconv (in_channels_ * kernel_size_ *
  ...
  bool FromFile (const char *fileprefix)
  {
    ...
    fread (&pconv[0], sizeof (float),
	   in_channels * out_channels * kernel_size * kernel_size, fp);
    fclose (fp);
...
struct FullyConnectedLayer
{
  int inputs, outputs;
    ...
    FullyConnectedLayer (int inputs_, int outputs_):outputs (outputs_),
    inputs (inputs_), pneurons (inputs_ * outputs_), pbias (outputs_)
  {
  }
...
struct TrainingContext
{
  ...
  cudnnTensorDescriptor_t dataTensor, conv1Tensor, conv1BiasTensor,
    pool1Tensor, conv2Tensor, conv2BiasTensor, pool2Tensor, fc1Tensor,
    fc2Tensor;
  ...
  //TrainingContext(TrainingContext&&) noexcept {}
    ...
    TrainingContext (int gpuid, int batch_size,
		     ...
		     FullyConnectedLayer * pfc1,
		     FullyConnectedLayer * pfc2):ref_pfc1 (pfc1),
    ref_pfc2 (pfc2), m_gpuid (gpuid)
  {
    ...
    checkCUDNN (cudnnSetActivationDescriptor
		(fc1Activation, CUDNN_ACTIVATION_RELU, CUDNN_PROPAGATE_NAN,
		 0.0));
    ...
    size_t workspace = 0;
      workspace =
      std::max (workspace,
		SetFwdConvolutionTensors (conv1, dataTensor, conv1Tensor,
					  conv1filterDesc, conv1Desc,
					  conv1algo));
      workspace =
      std::max (workspace,
		SetBwdConvolutionTensors (dataTensor, conv1Tensor,
					  conv1filterDesc, conv1Desc,
					  &conv1bwfalgo, nullptr));

      workspace =
      std::max (workspace,
		SetFwdConvolutionTensors (conv2, pool1Tensor, conv2Tensor,
					  conv2filterDesc, conv2Desc,
					  conv2algo));
      workspace =
      std::max (workspace,
		SetBwdConvolutionTensors (pool1Tensor, conv2Tensor,
					  conv2filterDesc, conv2Desc,
					  &conv2bwfalgo, &conv2bwdalgo));
  ...
  size_t SetFwdConvolutionTensors (ConvBiasLayer & conv,
...
//ssy: the size of each tensor is set here
  ...
  void ForwardPropagation1 (float *data, float *conv1, float *pool1,
			    float *conv2, float *pool2, float *fc1,
			    float *fc1relu, float *fc2, float *result,
			    float *pconv1, float *pconv1bias, float *pconv2,
			    float *pconv2bias, float *pfc1, float *pfc1bias,
			    float *pfc2, float *pfc2bias, void *workspace,
			    float *onevec)
  {
    ...
    checkCUDNN (cudnnConvolutionForward (cudnnHandle, &alpha, dataTensor,
					 data, conv1filterDesc, pconv1,
					 conv1Desc, conv1algo, workspace,
					 m_workspaceSize, &beta, conv1Tensor,
					 conv1));
    checkCUDNN (cudnnAddTensor
		(cudnnHandle, &alpha, conv1BiasTensor, pconv1bias, &alpha,
		 conv1Tensor, conv1));

    // Pool1 layer
    checkCUDNN (cudnnPoolingForward
		(cudnnHandle, poolDesc, &alpha, conv1Tensor, conv1, &beta,
		 pool1Tensor, pool1));

  }

  void ForwardPropagation2 (float *data, float *conv1, float *pool1,
			    float *conv2, float *pool2, float *fc1,
			    float *fc1relu, float *fc2, float *result,
			    float *pconv1, float *pconv1bias, float *pconv2,
			    float *pconv2bias, float *pfc1, float *pfc1bias,
			    ...
			    float *onevec)
  {
    ...
    checkCUDNN (cudnnConvolutionForward (cudnnHandle, &alpha, pool1Tensor,
					 pool1, conv2filterDesc, pconv2,
					 conv2Desc, conv2algo, workspace,
					 m_workspaceSize, &beta, conv2Tensor,
					 conv2));
    checkCUDNN (cudnnAddTensor
		(cudnnHandle, &alpha, conv2BiasTensor, pconv2bias, &alpha,
		 conv2Tensor, conv2));

    // Pool2 layer
    checkCUDNN (cudnnPoolingForward
		(cudnnHandle, poolDesc, &alpha, conv2Tensor, conv2, &beta,
		 pool2Tensor, pool2));
  }

  void ForwardPropagation3 (float *data, float *conv1, float *pool1,
			    float *conv2, float *pool2, float *fc1,
			    float *fc1relu, float *fc2, float *result,
			    float *pconv1, float *pconv1bias, float *pconv2,
			    float *pconv2bias, float *pfc1, float *pfc1bias,
...
//      printf("ref_pfc1->inputs %d\n",ref_pfc1->inputs);
    checkCudaErrors (cublasSgemm (cublasHandle, CUBLAS_OP_T, CUBLAS_OP_N,
				  ref_pfc1->outputs, m_batchSize,
				  ref_pfc1->inputs, &alpha, pfc1,
				  ref_pfc1->inputs, pool2, ref_pfc1->inputs,
				  &beta, fc1, ref_pfc1->outputs));
    // Add bias using GEMM's "beta" (fc1 += pfc1bias*1_vec')
    checkCudaErrors (cublasSgemm (cublasHandle, CUBLAS_OP_N, CUBLAS_OP_N,
				  ref_pfc1->outputs, m_batchSize, 1,
				  &alpha,
				  pfc1bias, ref_pfc1->outputs,
				  onevec, 1, &alpha, fc1, ref_pfc1->outputs));

    // ReLU activation
    checkCUDNN (cudnnActivationForward (cudnnHandle, fc1Activation, &alpha,
					fc1Tensor, fc1, &beta, fc1Tensor,
					fc1relu));

    // FC2 layer
    // Forward propagate neurons using weights (fc2 = pfc2'*fc1relu)
    checkCudaErrors (cublasSgemm (cublasHandle, CUBLAS_OP_T, CUBLAS_OP_N,
				  ref_pfc2->outputs, m_batchSize,
				  ref_pfc2->inputs, &alpha, pfc2,
				  ref_pfc2->inputs, fc1relu, ref_pfc2->inputs,
				  &beta, fc2, ref_pfc2->outputs));
    // Add bias using GEMM's "beta" (fc2 += pfc2bias*1_vec')
    checkCudaErrors (cublasSgemm (cublasHandle, CUBLAS_OP_N, CUBLAS_OP_N,
				  ref_pfc2->outputs, m_batchSize, 1,
				  &alpha,
				  pfc2bias, ref_pfc2->outputs,
				  onevec, 1, &alpha, fc2, ref_pfc2->outputs));

    // Softmax loss
    checkCUDNN (cudnnSoftmaxForward
		(cudnnHandle, CUDNN_SOFTMAX_ACCURATE,
		 CUDNN_SOFTMAX_MODE_CHANNEL, &alpha, fc2Tensor, fc2, &beta,
		 fc2Tensor, result));
  }

  size_t SetBwdConvolutionTensors (cudnnTensorDescriptor_t & srcTensorDesc,
				   cudnnTensorDescriptor_t & dstTensorDesc,
				   cudnnFilterDescriptor_t & filterDesc,
				   cudnnConvolutionDescriptor_t & convDesc,
				   ...
				   cudnnConvolutionBwdDataAlgo_t * dalgo)
  {
	...
	checkCUDNN (cudnnGetConvolutionBackwardFilterAlgorithm
		    (cudnnHandle, srcTensorDesc, dstTensorDesc, convDesc,
		     filterDesc, CUDNN_CONVOLUTION_BWD_FILTER_PREFER_FASTEST,
		     0, falgo));

	checkCUDNN (cudnnGetConvolutionBackwardFilterWorkspaceSize
		    (cudnnHandle, srcTensorDesc, dstTensorDesc, convDesc,
		     filterDesc, *falgo, &tmpsize));
	...
	checkCUDNN (cudnnGetConvolutionBackwardDataAlgorithm
		    (cudnnHandle, filterDesc, dstTensorDesc, convDesc,
		     srcTensorDesc, CUDNN_CONVOLUTION_BWD_DATA_PREFER_FASTEST,
		     0, dalgo));

	checkCUDNN (cudnnGetConvolutionBackwardDataWorkspaceSize
		    (cudnnHandle, filterDesc, dstTensorDesc, convDesc,
		     srcTensorDesc, *dalgo, &tmpsize));
  ...
  void Backpropagation (ConvBiasLayer & layer_conv1,
			MaxPoolLayer & layer_pool1,
			ConvBiasLayer & layer_conv2,
			MaxPoolLayer & layer_pool2, float *data,
			float *labels, float *conv1, float *pool1,
			float *conv2, float *pool2, float *fc1,
			float *fc1relu, float *fc2, float *fc2smax,
			float *dloss_data, float *pconv1, float *pconv1bias,
			float *pconv2, float *pconv2bias, float *pfc1,
			float *pfc1bias, float *pfc2, float *pfc2bias,
			float *gconv1, float *gconv1bias, float *dpool1,
			float *gconv2, float *gconv2bias, float *dconv2,
			float *dpool2, float *gfc1, float *gfc1bias,
			float *dfc1, float *dfc1relu, float *gfc2,
			float *gfc2bias, float *dfc2, void *workspace,
			float *onevec)
  {
    float alpha = 1.0f, beta = 0.0f;
    ...
    checkCudaErrors (cudaMemcpyAsync
		     (dloss_data, fc2smax,
		      sizeof (float) * m_batchSize * ref_pfc2->outputs,
		      cudaMemcpyDeviceToDevice));

    // Softmax layer
    SoftmaxLossBackprop <<< RoundUp (m_batchSize, BW), BW >>> (labels,
							       ref_pfc2->
							       outputs,
							       m_batchSize,
							       dloss_data);
    ...
    checkCudaErrors (cublasSscal
		     (cublasHandle, ref_pfc2->outputs * m_batchSize, &scalVal,
		      dloss_data, 1));
    ...
    checkCudaErrors (cublasSgemm
		     (cublasHandle, CUBLAS_OP_N, CUBLAS_OP_T,
		      ref_pfc2->inputs, ref_pfc2->outputs, m_batchSize,
		      &alpha, fc1relu, ref_pfc2->inputs, dloss_data,
		      ref_pfc2->outputs, &beta, gfc2, ref_pfc2->inputs));
    // Compute derivative with respect to bias: gfc2bias = dfc2smax * 1_vec
    checkCudaErrors (cublasSgemv
		     (cublasHandle, CUBLAS_OP_N, ref_pfc2->outputs,
		      m_batchSize, &alpha, dloss_data, ref_pfc2->outputs,
		      onevec, 1, &beta, gfc2bias, 1));
    // Compute derivative with respect to data (for previous layer): pfc2*dfc2smax (500x10*10xN)
    checkCudaErrors (cublasSgemm
		     (cublasHandle, CUBLAS_OP_N, CUBLAS_OP_N,
		      ref_pfc2->inputs, m_batchSize, ref_pfc2->outputs,
		      &alpha, pfc2, ref_pfc2->inputs, dloss_data,
		      ref_pfc2->outputs, &beta, dfc2, ref_pfc2->inputs));
    ...
    checkCudaErrors (cublasSgemm
		     (cublasHandle, CUBLAS_OP_N, CUBLAS_OP_T,
		      ref_pfc1->inputs, ref_pfc1->outputs, m_batchSize,
		      &alpha, pool2, ref_pfc1->inputs, dfc1relu,
		      ref_pfc1->outputs, &beta, gfc1, ref_pfc1->inputs));
    // Compute derivative with respect to bias: gfc1bias = dfc1relu * 1_vec
    checkCudaErrors (cublasSgemv
		     (cublasHandle, CUBLAS_OP_N, ref_pfc1->outputs,
		      m_batchSize, &alpha, dfc1relu, ref_pfc1->outputs,
		      onevec, 1, &beta, gfc1bias, 1));
    // Compute derivative with respect to data (for previous layer): pfc1*dfc1relu (800x500*500xN)
    checkCudaErrors (cublasSgemm
		     (cublasHandle, CUBLAS_OP_N, CUBLAS_OP_N,
		      ref_pfc1->inputs, m_batchSize, ref_pfc1->outputs,
		      &alpha, pfc1, ref_pfc1->inputs, dfc1relu,
		      ref_pfc1->outputs, &beta, dfc1, ref_pfc1->inputs));

    // Pool2 layer
    checkCUDNN (cudnnPoolingBackward (cudnnHandle, poolDesc, &alpha,
				      pool2Tensor, pool2, pool2Tensor, dfc1,
				      conv2Tensor, conv2, &beta, conv2Tensor,
				      dpool2));

    // Conv2 layer
    checkCUDNN (cudnnConvolutionBackwardBias
		(cudnnHandle, &alpha, conv2Tensor, dpool2, &beta,
		 conv2BiasTensor, gconv2bias));


    checkCUDNN (cudnnConvolutionBackwardFilter
		(cudnnHandle, &alpha, pool1Tensor, pool1, conv2Tensor, dpool2,
		 conv2Desc, conv2bwfalgo, workspace, m_workspaceSize, &beta,
		 conv2filterDesc, gconv2));

    checkCUDNN (cudnnConvolutionBackwardData
		(cudnnHandle, &alpha, conv2filterDesc, pconv2, conv2Tensor,
		 dpool2, conv2Desc, conv2bwdalgo, workspace, m_workspaceSize,
		 &beta, pool1Tensor, dconv2));

    // Pool1 layer
    checkCUDNN (cudnnPoolingBackward (cudnnHandle, poolDesc, &alpha,
				      pool1Tensor, pool1, pool1Tensor, dconv2,
				      conv1Tensor, conv1, &beta, conv1Tensor,
				      dpool1));

    // Conv1 layer
    checkCUDNN (cudnnConvolutionBackwardBias
		(cudnnHandle, &alpha, conv1Tensor, dpool1, &beta,
		 conv1BiasTensor, gconv1bias));

    checkCUDNN (cudnnConvolutionBackwardFilter
		(cudnnHandle, &alpha, dataTensor, data, conv1Tensor, dpool1,
		 conv1Desc, conv1bwfalgo, workspace, m_workspaceSize, &beta,
		      ...
		      float *gfc2, float *gfc2bias)
  {
    ...
    checkCudaErrors (cublasSaxpy
		     (cublasHandle, static_cast < int >(conv1.pconv.size ()),
		      &alpha, gconv1, 1, pconv1, 1));
    checkCudaErrors (cublasSaxpy
		     (cublasHandle, static_cast < int >(conv1.pbias.size ()),
		      &alpha, gconv1bias, 1, pconv1bias, 1));

    // Conv2
    checkCudaErrors (cublasSaxpy
		     (cublasHandle, static_cast < int >(conv2.pconv.size ()),
		      &alpha, gconv2, 1, pconv2, 1));
    checkCudaErrors (cublasSaxpy
		     (cublasHandle, static_cast < int >(conv2.pbias.size ()),
		      &alpha, gconv2bias, 1, pconv2bias, 1));

    // Fully connected 1
    checkCudaErrors (cublasSaxpy
		     (cublasHandle,
		      static_cast < int >(ref_pfc1->pneurons.size ()), &alpha,
		      gfc1, 1, pfc1, 1));
    checkCudaErrors (cublasSaxpy
		     (cublasHandle,
		      static_cast < int >(ref_pfc1->pbias.size ()), &alpha,
		      gfc1bias, 1, pfc1bias, 1));

    // Fully connected 2
    checkCudaErrors (cublasSaxpy
		     (cublasHandle,
		      static_cast < int >(ref_pfc2->pneurons.size ()), &alpha,
		      gfc2, 1, pfc2, 1));
    checkCudaErrors (cublasSaxpy
		     (cublasHandle,
		      static_cast < int >(ref_pfc2->pbias.size ()), &alpha,
		      gfc2bias, 1, pfc2bias, 1));
  }
};
...
//ssy the temp buf to hold the image
  std::vector < uint8_t >
    train_images (train_size * width * height * channels),
  ...
  for (int gpuid = 0; gpuid < num_gpus; gpuid++)
    {
      ...
      FullyConnectedLayer *pfc1 =
	new
	FullyConnectedLayer ((conv2.out_channels * conv2.out_width *
			      conv2.out_height) / (pool2.stride *
						   pool2.stride), 500);
      FullyConnectedLayer *pfc2 = new FullyConnectedLayer (pfc1->outputs, 10);
      TrainingContext *pcontext =
	new TrainingContext (gpuid, FLAGS_batch_size, conv1, pool1, conv2,
			     pool2, pfc1, pfc2);
      ...
      if (!bRet || !FLAGS_pretrained)
	{
	  ...
	  std::mt19937 gen (FLAGS_random_seed < 0 ? rd () : static_cast <
			    unsigned int >(FLAGS_random_seed));

	  // Xavier weight filling
	  float wconv1 =
	    sqrt (3.0f /
		  (conv1.kernel_size * conv1.kernel_size *
		   conv1.in_channels));
	  std::uniform_real_distribution <> dconv1 (-wconv1, wconv1);
	  float wconv2 =
	    sqrt (3.0f /
		  (conv2.kernel_size * conv2.kernel_size *
		   conv2.in_channels));
	...
	for (auto && iter:pfc2->pbias)
      ...
      checkCudaErrors (cudaMallocManaged
		       (&d_data,
			sizeof (float) * pcontext->m_batchSize * channels *
			height * width));
      checkCudaErrors (cudaMallocManaged
		       (&d_labels,
			sizeof (float) * pcontext->m_batchSize * 1 * 1 * 1));
      checkCudaErrors (cudaMallocManaged
		       (&d_conv1,
			sizeof (float) * pcontext->m_batchSize *
      ...
      checkCudaErrors (cudaMallocManaged
		       (&d_pconv1, sizeof (float) * conv1.pconv.size ()));
      checkCudaErrors (cudaMallocManaged
		       (&d_pconv1bias, sizeof (float) * conv1.pbias.size ()));
      checkCudaErrors (cudaMallocManaged
		       (&d_pconv2, sizeof (float) * conv2.pconv.size ()));
      checkCudaErrors (cudaMallocManaged
		       (&d_pconv2bias, sizeof (float) * conv2.pbias.size ()));
      checkCudaErrors (cudaMallocManaged
		       (&d_pfc1, sizeof (float) * pfc1->pneurons.size ()));
      checkCudaErrors (cudaMallocManaged
		       (&d_pfc1bias, sizeof (float) * pfc1->pbias.size ()));
      checkCudaErrors (cudaMallocManaged
		       (&d_pfc2, sizeof (float) * pfc2->pneurons.size ()));
      checkCudaErrors (cudaMallocManaged
		       (&d_pfc2bias, sizeof (float) * pfc2->pbias.size ()));
      ...
      checkCudaErrors (cudaMallocManaged
		       (&d_gconv1, sizeof (float) * conv1.pconv.size ()));
      checkCudaErrors (cudaMallocManaged
		       (&d_gconv1bias, sizeof (float) * conv1.pbias.size ()));
      checkCudaErrors (cudaMallocManaged
		       (&d_gconv2, sizeof (float) * conv2.pconv.size ()));
      checkCudaErrors (cudaMallocManaged
		       (&d_gconv2bias, sizeof (float) * conv2.pbias.size ()));
      checkCudaErrors (cudaMallocManaged
		       (&d_gfc1, sizeof (float) * pfc1->pneurons.size ()));
      checkCudaErrors (cudaMallocManaged
		       (&d_gfc1bias, sizeof (float) * pfc1->pbias.size ()));
      checkCudaErrors (cudaMallocManaged
		       (&d_gfc2, sizeof (float) * pfc2->pneurons.size ()));
      checkCudaErrors (cudaMallocManaged
		       (&d_gfc2bias, sizeof (float) * pfc2->pbias.size ()));

      checkCudaErrors (cudaMemAdvise (d_data,sizeof(float)*
				    pcontext->m_batchSize * channels *
				    height * width,cudaMemAdviseSetPreferredLocation,gpuid));
      checkCudaErrors (cudaMemAdvise (d_labels,sizeof(float)*
				    pcontext->m_batchSize * 1 * 1 * 1,cudaMemAdviseSetPreferredLocation,gpuid));
      checkCudaErrors (cudaMemAdvise (d_conv1,sizeof(float)*
				    pcontext->m_batchSize *
				    conv1.out_channels * conv1.out_height *
				    conv1.out_width,cudaMemAdviseSetPreferredLocation,gpuid));
      checkCudaErrors (cudaMemAdvise (d_pool1,sizeof(float)*
				    pcontext->m_batchSize *
				    conv1.out_channels * (conv1.out_height /
							  pool1.stride) *
				    (conv1.out_width / pool1.stride),cudaMemAdviseSetPreferredLocation,gpuid));
      checkCudaErrors (cudaMemAdvise (d_pool1_xxx,sizeof(float)*
				    pcontext->m_batchSize *
				    conv1.out_channels * (conv1.out_height /
							  pool1.stride) *
				    (conv1.out_width / pool1.stride),cudaMemAdviseSetPreferredLocation,gpuid));
      checkCudaErrors (cudaMemAdvise (d_conv2,sizeof(float)*
				    pcontext->m_batchSize *
				    conv2.out_channels * conv2.out_height *
				    conv2.out_width,cudaMemAdviseSetPreferredLocation,gpuid));
      checkCudaErrors (cudaMemAdvise (d_pool2,sizeof(float)*
				    pcontext->m_batchSize *
				    conv2.out_channels * (conv2.out_height /
							  pool2.stride) *
				    (conv2.out_width / pool2.stride),cudaMemAdviseSetPreferredLocation,gpuid));
      checkCudaErrors (cudaMemAdvise (d_pool2_xxx,sizeof(float)*
				    pcontext->m_batchSize *
				    conv2.out_channels * (conv2.out_height /
							  pool2.stride) *
				    (conv2.out_width / pool2.stride),cudaMemAdviseSetPreferredLocation,gpuid));
      checkCudaErrors (cudaMemAdvise (d_fc1,sizeof(float)*
				    pcontext->m_batchSize * pfc1->outputs,cudaMemAdviseSetPreferredLocation,gpuid));
      checkCudaErrors (cudaMemAdvise (d_fc1relu,sizeof(float)*
				    pcontext->m_batchSize * pfc1->outputs,cudaMemAdviseSetPreferredLocation,gpuid));
      checkCudaErrors (cudaMemAdvise (d_fc2,sizeof(float)*
				    pcontext->m_batchSize * pfc2->outputs,cudaMemAdviseSetPreferredLocation,gpuid));
      checkCudaErrors (cudaMemAdvise (d_fc2smax,sizeof(float)*
				    pcontext->m_batchSize * pfc2->outputs,cudaMemAdviseSetPreferredLocation,gpuid));

      checkCudaErrors (cudaMemAdvise (d_pconv1,sizeof(float)* conv1.pconv.size (),cudaMemAdviseSetPreferredLocation,gpuid));
      checkCudaErrors (cudaMemAdvise (d_pconv1bias,sizeof(float)* conv1.pbias.size (),cudaMemAdviseSetPreferredLocation,gpuid));
      checkCudaErrors (cudaMemAdvise (d_pconv2,sizeof(float)* conv2.pconv.size (),cudaMemAdviseSetPreferredLocation,gpuid));
      checkCudaErrors (cudaMemAdvise (d_pconv2bias,sizeof(float)* conv2.pbias.size (),cudaMemAdviseSetPreferredLocation,gpuid));
      checkCudaErrors (cudaMemAdvise (d_pfc1,sizeof(float)* pfc1->pneurons.size (),cudaMemAdviseSetPreferredLocation,gpuid));
      checkCudaErrors (cudaMemAdvise (d_pfc1bias,sizeof(float)* pfc1->pbias.size (),cudaMemAdviseSetPreferredLocation,gpuid));
      checkCudaErrors (cudaMemAdvise (d_pfc2,sizeof(float)* pfc2->pneurons.size (),cudaMemAdviseSetPreferredLocation,gpuid));
      checkCudaErrors (cudaMemAdvise (d_pfc2bias,sizeof(float)* pfc2->pbias.size (),cudaMemAdviseSetPreferredLocation,gpuid));

      checkCudaErrors (cudaMemAdvise (d_gconv1,sizeof(float)* conv1.pconv.size (),cudaMemAdviseSetPreferredLocation,gpuid));
      checkCudaErrors (cudaMemAdvise (d_gconv1bias,sizeof(float)* conv1.pbias.size (),cudaMemAdviseSetPreferredLocation,gpuid));
      checkCudaErrors (cudaMemAdvise (d_gconv2,sizeof(float)* conv2.pconv.size (),cudaMemAdviseSetPreferredLocation,gpuid));
      checkCudaErrors (cudaMemAdvise (d_gconv2bias,sizeof(float)* conv2.pbias.size (),cudaMemAdviseSetPreferredLocation,gpuid));
      checkCudaErrors (cudaMemAdvise (d_gfc1,sizeof(float)* pfc1->pneurons.size (),cudaMemAdviseSetPreferredLocation,gpuid));
      checkCudaErrors (cudaMemAdvise (d_gfc1bias,sizeof(float)* pfc1->pbias.size (),cudaMemAdviseSetPreferredLocation,gpuid));
...
//    checkCudaErrors(cudaMallocManaged(&d_dlossdata,sizeof(float) * pcontext->m_batchSize * pfc2->outputs));
      ...
      float *d_onevec;
      checkCudaErrors (cudaMallocManaged
		       (&d_onevec, sizeof (float) * pcontext->m_batchSize));
      d_onevecV.push_back (d_onevec);

      void *d_cudnn_workspace = nullptr;
      if (pcontext->m_workspaceSize > 0)
	checkCudaErrors (cudaMallocManaged
			 (&d_cudnn_workspace, pcontext->m_workspaceSize));
...
//    checkCudaErrors(cudaMemcpyAsync(d_pfc2bias, &pfc2->pbias[0],     sizeof(float) * pfc2->pbias.size(),    cudaMemcpyHostToDevice));
      ...
      FillOnes <<< RoundUp (pcontext->m_batchSize, BW), BW >>> (d_onevec,
								pcontext->
								m_batchSize);
...
//        train_labels_float[i] = (float)train_labels[i];
  ...
  printf ("conv1 output dim %d %d %d %d\n", contextV[0]->m_batchSize,
	  conv1V[0].out_channels, conv1V[0].out_height, conv1V[0].out_width);
  printf ("pool1 output dim %d %d %d %d\n", contextV[0]->m_batchSize,
	  conv1V[0].out_channels, (conv1V[0].out_height / pool1V[0].stride),
	  (conv1V[0].out_width / pool1V[0].stride));
  printf ("conv2 outut dim %d %d %d %d\n", contextV[0]->m_batchSize,
	  conv2V[0].out_channels, conv2V[0].out_height, conv2V[0].out_width);
  printf ("pool2 output dim %d %d %d %d\n", contextV[0]->m_batchSize,
	  conv2V[0].out_channels, (conv2V[0].out_height / pool2V[0].stride),
	  (conv2V[0].out_width / pool2V[0].stride));
  ...
  for (int gpuid = 0; gpuid < num_gpus; gpuid++)
	{
	  ...
	  checkCudaErrors (cudaSetDevice (gpuid));
	  contextV[gpuid]->ForwardPropagation1 (d_dataV[gpuid],
						d_conv1V[gpuid],
						d_pool1V[gpuid],
						d_conv2V[gpuid],
						d_pool2V[gpuid],
...
//                  checkCudaErrors(cudaMemcpyAsync(d_pool1V[gpuid], d_pool1V[gpuid+1], int(fract*sz/2), cudaMemcpyDefault));
  ...
  for (int gpuid = 0; gpuid < num_gpus; gpuid++)
	{
	  checkCudaErrors (cudaSetDevice (gpuid));
	  contextV[gpuid]->ForwardPropagation2 (d_dataV[gpuid],
						d_conv1V[gpuid],
						d_pool1V[gpuid],
						d_conv2V[gpuid],
						d_pool2V[gpuid],

...
//        return 3;
    ...
    */
    ...
    //-----------------------------------------------------------------------------------------------------------------------------------------
    checkCudaErrors(cudaMallocManaged(&d_data,    sizeof(float) * context.m_batchSize * channels           * height                            * width));
    checkCudaErrors(cudaMallocManaged(&d_labels,  sizeof(float) * context.m_batchSize * 1                  * 1                                 * 1));
    checkCudaErrors(cudaMallocManaged(&d_conv1,   sizeof(float) * context.m_batchSize * conv1.out_channels * conv1.out_height                  * conv1.out_width));
    checkCudaErrors(cudaMallocManaged(&d_pool1,   sizeof(float) * context.m_batchSize * conv1.out_channels * (conv1.out_height / pool1.stride) * (conv1.out_width / pool1.stride)));
    checkCudaErrors(cudaMallocManaged(&d_conv2,   sizeof(float) * context.m_batchSize * conv2.out_channels * conv2.out_height                  * conv2.out_width));
    checkCudaErrors(cudaMallocManaged(&d_pool2,   sizeof(float) * context.m_batchSize * conv2.out_channels * (conv2.out_height / pool2.stride) * (conv2.out_width / pool2.stride)));
    checkCudaErrors(cudaMallocManaged(&d_fc1,     sizeof(float) * context.m_batchSize * fc1.outputs));    
    checkCudaErrors(cudaMallocManaged(&d_fc1relu, sizeof(float) * context.m_batchSize * fc1.outputs));
    checkCudaErrors(cudaMallocManaged(&d_fc2,     sizeof(float) * context.m_batchSize * fc2.outputs));
    checkCudaErrors(cudaMallocManaged(&d_fc2smax, sizeof(float) * context.m_batchSize * fc2.outputs));    
    ...
    checkCudaErrors(cudaMallocManaged(&d_pfc1,       sizeof(float) * fc1.pneurons.size()));
    checkCudaErrors(cudaMallocManaged(&d_pfc1bias,   sizeof(float) * fc1.pbias.size()));
    checkCudaErrors(cudaMallocManaged(&d_pfc2,       sizeof(float) * fc2.pneurons.size()));
    checkCudaErrors(cudaMallocManaged(&d_pfc2bias,   sizeof(float) * fc2.pbias.size()));    
    ...
    checkCudaErrors(cudaMallocManaged(&d_gfc1,       sizeof(float) * fc1.pneurons.size()));
    checkCudaErrors(cudaMallocManaged(&d_gfc1bias,   sizeof(float) * fc1.pbias.size()));    
    checkCudaErrors(cudaMallocManaged(&d_gfc2,       sizeof(float) * fc2.pneurons.size()));
    checkCudaErrors(cudaMallocManaged(&d_gfc2bias,   sizeof(float) * fc2.pbias.size()));
...
//    //                         Buffer     | Element       | N                   | C                  | H                                 | W
//    //-----------------------------------------------------------------------------------------------------------------------------------------
//    checkCudaErrors(cudaMallocManaged(&d_dpool1,   sizeof(float) * context.m_batchSize * conv1.out_channels * conv1.out_height                  * conv1.out_width));
//    checkCudaErrors(cudaMallocManaged(&d_dpool2,   sizeof(float) * context.m_batchSize * conv2.out_channels * conv2.out_height                  * conv2.out_width));
//    checkCudaErrors(cudaMallocManaged(&d_dconv2,   sizeof(float) * context.m_batchSize * conv1.out_channels * (conv1.out_height / pool1.stride) * (conv1.out_width / pool1.stride)));
//    checkCudaErrors(cudaMallocManaged(&d_dfc1,     sizeof(float) * context.m_batchSize * fc1.inputs));
//    checkCudaErrors(cudaMallocManaged(&d_dfc1relu, sizeof(float) * context.m_batchSize * fc1.outputs));
//    checkCudaErrors(cudaMallocManaged(&d_dfc2,     sizeof(float) * context.m_batchSize * fc2.inputs));
//    checkCudaErrors(cudaMallocManaged(&d_dfc2smax, sizeof(float) * context.m_batchSize * fc2.outputs));
//    checkCudaErrors(cudaMallocManaged(&d_dlossdata,sizeof(float) * context.m_batchSize * fc2.outputs));
    
    // Temporary buffers and workspaces
    float *d_onevec;
    void *d_cudnn_workspace = nullptr;    
    checkCudaErrors(cudaMallocManaged(&d_onevec, sizeof(float)* context.m_batchSize));
    if (context.m_workspaceSize > 0)
        checkCudaErrors(cudaMallocManaged(&d_cudnn_workspace, context.m_workspaceSize));    
...
//        train_labels_float[i] = (float)train_labels[i];
	...
	printf("iteration %d\n",iterations);
	printf("batch size  %d\n",context.m_batchSize);
	printf("input data dimension %d %d  %d %d\n",context.m_batchSize,channels,height,width);
	printf("conv1 output dim %d %d %d %d\n",context.m_batchSize , conv1.out_channels, conv1.out_height, conv1.out_width);
	printf("pool1 output dim %d %d %d %d\n",context.m_batchSize , conv1.out_channels , (conv1.out_height / pool1.stride) , (conv1.out_width / pool1.stride));
	printf("conv2 outut dim %d %d %d %d\n",context.m_batchSize, conv2.out_channels , conv2.out_height  , conv2.out_width);
	printf("pool2 output dim %d %d %d %d\n",context.m_batchSize , conv2.out_channels , (conv2.out_height / pool2.stride) , (conv2.out_width / pool2.stride));
	printf("fc1 out dim  %d %d\n",context.m_batchSize , fc1.outputs);
	printf("fc1relu out dim  %d %d\n",context.m_batchSize , fc1.outputs);
	printf("fc2 out dim  %d %d\n",context.m_batchSize , fc2.outputs);

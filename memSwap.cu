...
void allocAdviseMemoryWithHost(void** ppout,void** ppoutHost,size_t outbuf_size,int gpuid) {
	assert(outbuf_size>0);
	checkCudaErrors(cudaSetDevice(gpuid));
  checkCudaErrors (cudaMallocManaged (ppout, outbuf_size));
  //checkCudaErrors (cudaMallocManaged (ppoutHost, outbuf_size));
	*ppoutHost = malloc(outbuf_size);
	if(advise) {
	  checkCudaErrors (cudaMemAdvise (*ppout,outbuf_size ,cudaMemAdviseSetPreferredLocation,gpuid));
	  //checkCudaErrors (cudaMemAdvise (*ppoutHost,outbuf_size ,cudaMemAdviseSetPreferredLocation,cudaCpuDeviceId));
	}
//	float ** pp=(float**)ppoutHost;
//	float *  p =*pp;
//	for(int i=0;i<outbuf_size/sizeof(float);i++) {
//		p[i]=(float )0.0;
//	}
...
class baseModule{
  ...
  int in_width, in_height, out_width, out_height;
	...
	float * poutHost; // pout should be alloc by child class
	float * poutDiff; //pass in from outside 
	long outbuf_size;
...
//		out_height = out_h_;
		...
		allocAdviseMemoryWithHost((void**)&pout,(void**)&poutHost,sizeof (float) *outbuf_size,gpuid);
	}

	void allocPinDiff() {
		allocAdviseMemoryWithHost((void**)&pinDiff,(void**)&pinDiffHost,sizeof (float) *inbuf_size,gpuid);
	}
...
class ConvBiasLayer: public baseModule
{
		...
		float * pconvbiasGradientHost;

		cudnnTensorDescriptor_t srcTensorDesc;

		cudnnFilterDescriptor_t filterDesc;
		float * pconvWeigth;
		float * pconvWeigthHost;
		float * pconvWeigthGradient;
		float * pconvWeigthGradientHost;
    ...
    ConvBiasLayer (
								...
								)
		{
    		...
    		allocAdviseMemoryWithHost((void**)&pconvbias,(void**)&pconvbiasHost ,        sizeof(float) * out_channels,gpuid );
    		allocAdviseMemoryWithHost((void**)&pconvbiasGradient,(void**)&pconvbiasGradientHost, sizeof(float) * out_channels,gpuid );
    		...
    		checkCUDNN (cudnnSetFilter4dDescriptor (filterDesc,
					    ...
					    kernel_size,
					    kernel_size));
				allocAdviseMemoryWithHost((void**)&pconvWeigth, (void**)&pconvWeigthHost,       sizeof(float)*in_channels_*kernel_size_*kernel_size_*numFilter_,gpuid);
				allocAdviseMemoryWithHost((void**)&pconvWeigthGradient,(void**)&pconvWeigthGradientHost,sizeof(float)*in_channels_*kernel_size_*kernel_size_*numFilter_,gpuid);
...
class TrainingContext
{
  ...
  TrainingContext (int gpuid, int batch_size,float alpha_,float beta_)
  {
	...
	baseModule * getPrevLayer() {
		assert(hasPrevLayer());
		return vmod[currentlayer-1];
	}
	baseModule * getNextLayer() {
		assert(hasNextLayer());
		return vmod[currentlayer+1];
	}
	...
	bool hasPrevLayer() {
		if(currentlayer>=1) return true;
		else return false;
	}
	bool hasNextLayer() {
		if(currentlayer<vmod.size()-1) return true;
		else return false;
	}
...
{
  if (argc != 9) {
		printf("Usage : cudnnModelParallel.exe <nettype> <iteration> <minbatch> <width>  <copy or not> <fract to copy> <advise or not> <gpu number>");
		assert(0);
	}
		  ...
		  if (copy)
			{
			  for (int gpuid = 0; gpuid < num_gpus; gpuid++)
			  {
			      ...
			      if (contextV[gpuid]->hasPrevLayer()) {
							baseModule * pPrev =contextV[gpuid]->getPrevLayer();
							size_t szPrev = sizeof (float) * (pPrev->getOutputFloatNumber() );
							assert(sz==szPrev);
							size_t tobetransfered =sz; //all must be backuped
							if(gpuid == num_gpus-1)
								totalsize = totalsize+tobetransfered;
							//backup feature
						  checkCudaErrors (cudaMemcpyAsync (pPrev->poutHost , pcurrent->pin, tobetransfered, cudaMemcpyDefault));
						}
			  }
		  ...
		  if (copy)
			{
			  for (int gpuid = 0; gpuid < num_gpus; gpuid++)
			  {
			      ...
			      if (contextV[gpuid]->hasNextLayer() && contextV[gpuid]->hasPrevLayer()) {
							baseModule * pNext =contextV[gpuid]->getNextLayer();
							size_t szNext = sizeof (float) * (pNext->getInputFloatNumber() );
							assert(sz==szNext);
							size_t tobetransfered = sz;
							if(gpuid == num_gpus-1)
								totalsize = totalsize+tobetransfered;
							//back up diff freature
						  checkCudaErrors (cudaMemcpyAsync (pNext->pinDiffHost, pcurrent->poutDiff, tobetransfered, cudaMemcpyDefault));
							//copy back feature
							baseModule * pPrev =contextV[gpuid]->getPrevLayer();
							size_t szPrev = sizeof (float) * (pPrev->getOutputFloatNumber() );
							tobetransfered=szPrev;
						  checkCudaErrors (cudaMemcpyAsync (pcurrent->pin,pPrev->poutHost, tobetransfered, cudaMemcpyDefault));
						} else {
